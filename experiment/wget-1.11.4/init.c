...
#else
  opt.restrict_files_os = options::restrict_unix;
#endif
  opt.restrict_files_ctrl = true;
  opt.restrict_files_case = options::restrict_no_case_restriction;
...
{
  if (!simple_atoi (val, val + strlen (val), (int *)place)
      || *(int *) place < 0)
    {
...
{
  static const struct decode_item choices[] = {
    { "IPv4", options::prefer_ipv4 },
    { "IPv6", options::prefer_ipv6 },
    { "none", options::prefer_none },
  };
  int prefer_family = options::prefer_ipv4;
  int ok = decode_string (val, choices, countof (choices), &prefer_family);
  if (!ok)
    fprintf (stderr, _("%s: %s: Invalid value `%s'.\n"), exec_name, com, val);
  opt.prefer_family = (enum options::prefer_family) prefer_family;
  return ok;
}
  ...
  do
    {
      ...
      if (VAL_IS ("unix"))
        restrict_os = options::restrict_unix;
      else if (VAL_IS ("windows"))
        restrict_os = options::restrict_windows;
      else if (VAL_IS ("lowercase"))
        restrict_case = options::restrict_lowercase;
      else if (VAL_IS ("uppercase"))
        restrict_case = options::restrict_uppercase;
...
#undef VAL_IS
  ...
  opt.restrict_files_os = (enum options::restrict_files_os) restrict_os;
  opt.restrict_files_ctrl = restrict_ctrl;
  opt.restrict_files_case = (enum options::restrict_files_case) restrict_case;
