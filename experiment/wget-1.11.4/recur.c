...
url_queue_new (void)
{
  struct url_queue *queue = (url_queue *)xnew0 (struct url_queue);
  return queue;
}
...
{
  struct queue_element *qel = (queue_element *)xnew (struct queue_element);
  qel->url = url;
  qel->referer = referer;
  qel->depth = depth;
  ...
  while (1)
    {
      ...
      if (dl_url_file_map && hash_table_contains (dl_url_file_map, url))
        {
          file = xstrdup ((char *)hash_table_get (dl_url_file_map, url));
                    ...
                    struct url *start_url_parsed, struct hash_table *blacklist)
{
  ...
  upos = (urlpos *) xnew0 (struct urlpos);
  upos->url = new_parsed;
