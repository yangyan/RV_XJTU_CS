...
address_list_from_addrinfo (const struct addrinfo *ai)
{
  ...
  al = (address_list *)xnew0 (struct address_list);
  al->addresses = (ip_address *)xnew_array (ip_address, cnt);
  al->count     = cnt;
  al->refcount  = 1;
...
cache_query (const char *host)
{
  ...
  if (!host_name_addresses_map)
    return NULL;
  al = (address_list *)hash_table_get (host_name_addresses_map, host);
  if (al)
    {
...
lookup_host (const char *host, int flags)
{
...
#ifdef ENABLE_IPV6
  {
    ...
    if (al->count > 1 && opt.prefer_family != options::prefer_none)
      stable_sort (al->addresses, al->count, sizeof (ip_address),
                   opt.prefer_family == options::prefer_ipv4
  ...
  if (host_name_addresses_map)
    {
      ...
      for (hash_table_iterate (host_name_addresses_map, &iter);
        ...
        {
          char *host = (char *)iter.key;
          struct address_list *al = (address_list *)iter.value;
          xfree (host);
