  ...
  for (i = 0; i < cnt; i++)
    {
      ...
      /* Determine the URL of the HTML file.  get_urls_html will need
         it.  */
      url = (char *)hash_table_get (dl_file_url_map, file);
      if (!url)
        {
      ...
      for (cur_url = urls; cur_url; cur_url = cur_url->next)
        {
          ...
          local_name = (char *)hash_table_get (dl_url_file_map, u->url);
  ...
  if (downloaded_file_return == FILE_DOWNLOADED_AND_HTML_EXTENSION_ADDED)
    {
      ...
      filename_plus_orig_suffix = (char *)alloca (filename_len + 1);
  ...
  else /* downloaded_file_return == FILE_DOWNLOADED_NORMALLY */
    {
      ...
      filename_plus_orig_suffix = (char *)alloca (filename_len + sizeof (".orig"));
      strcpy (filename_plus_orig_suffix, file);
...
register_redirection (const char *from, const char *to)
{
  ...
  file = (char *)hash_table_get (dl_url_file_map, to);
  assert (file != NULL);
  ...
  if (mode == CHECK_FOR_FILE)
    {
      ...
      ptr = (downloaded_file_t *)hash_table_get (downloaded_files_hash, file);
      if (!ptr)
        return FILE_NOT_ALREADY_DOWNLOADED;
      return *ptr;
    }
