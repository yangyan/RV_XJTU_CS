...
fd_register_transport (int fd, struct transport_implementation *imp, void *ctx)
{
  ...
  info = (transport_info *)xnew (struct transport_info);
  info->imp = imp;
  info->ctx = ctx;
  if (!transport_map)
...
{
  struct transport_info *info = (transport_info *)hash_table_get (transport_map, (void *)(intptr_t) fd);
  return info->ctx;
}
...
#define LAZY_RETRIEVE_INFO(info) do {                                   \
  ...
  if (!transport_map)                                                   \
    ...
    {                                                                   \
      info = (transport_info *)hash_table_get (transport_map, (void *)(intptr_t) fd);     \
      last_fd = fd;                                                     \
      last_info = info;                                                 \
