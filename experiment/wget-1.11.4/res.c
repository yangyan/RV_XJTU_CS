  ...
  if (specs->count > specs->size)
    {
      if (specs->size == 0)
        ...
        specs->size <<= 1;
      specs->paths = (path_info *)xrealloc (specs->paths,
...
prune_non_exact (struct robot_specs *specs)
{
  ...
  for (i = 0; i < specs->count; i++)
    if (specs->paths[i].user_agent_exact_p)
      ++cnt;
  newpaths = (path_info *)xnew_array (struct path_info, cnt);
...
res_parse (const char *source, int length)
{
  ...
  struct robot_specs *specs = (robot_specs *)xnew0 (struct robot_specs);
...
#define SET_HOSTPORT(host, port, result) do {           \
  ...
  result = (char *)alloca (HP_len + 1 + numdigit (port) + 1);   \
...
res_get_specs (const char *host, int port)
{
  ...
  if (!registered_specs)
    return NULL;
  return (robot_specs *)hash_table_get (registered_specs, hp);
}

  ...
  if (registered_specs)
    {
      ...
      for (hash_table_iterate (registered_specs, &iter);
        ...
        {
          xfree (iter.key);
          free_specs ((robot_specs *)iter.value);
        }
