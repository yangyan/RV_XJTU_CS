...
{
  int size;
  struct hash_table *ht = (hash_table *)xnew (struct hash_table);
  ...
  ht->cells = (cell *)xnew_array (struct cell, ht->size);
...
grow_hash_table (struct hash_table *ht)
{
  ...
  cells = (cell *)xnew_array (struct cell, newsize);
...
{
  struct cell *c = (cell *)iter->pos;
  struct cell *end = (cell *)iter->end;
  for (; c < end; c++)
    if (CELL_OCCUPIED (c))
      {
        iter->key = c->key;
