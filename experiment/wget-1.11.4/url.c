...
#define FN_PORT_SEP  (opt.restrict_files_os != options::restrict_windows ? ':' : '+')

/* FN_QUERY_SEP is the separator between the file name and the URL
   query, normally '?'.  Since Windows cannot handle '?' as part of
   file name, we use '@' instead there.  */
#define FN_QUERY_SEP (opt.restrict_files_os != options::restrict_windows ? '?' : '@')
                   ...
                   struct growable *dest)
{
  ...
  int mask;
  if (opt.restrict_files_os == options::restrict_unix)
  ...
  if (opt.restrict_files_case == options::restrict_lowercase
      || opt.restrict_files_case == options::restrict_uppercase)
    {
      char *q;
...
uri_merge (const char *base, const char *link)
{
  ...
  if (!*link)
  ...
  else
    {
      ...
      int span;
      const char *start_insert;
      const char *last_slash = (char *)find_last_char (base, end, '/');
      if (!last_slash)
        {
