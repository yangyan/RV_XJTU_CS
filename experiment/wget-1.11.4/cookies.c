...
cookie_jar_new (void)
{
  struct cookie_jar *jar = (cookie_jar *)xnew (struct cookie_jar);
...
struct cookie {
  ...
  cookie *next;          /* used for chaining of cookies in the
                                   same domain. */
};
...
cookie_new (void)
{
  struct cookie *cookie = (struct cookie *)xnew0 (cookie);
                      ...
                      struct cookie **prevptr)
{
  ...
  chain = (struct cookie *)hash_table_get (jar->chains, cookie->domain);
  if (!chain)
    goto nomatch;
  ...
  if (!cookie->path)
    {
      ...
      char *trailing_slash = strrchr ((char *)path, '/');
      if (trailing_slash)
                     ...
                     struct cookie *dest[])
{
  ...
  while (1)
    {
      struct cookie *chain = (struct cookie *)hash_table_get (jar->chains, host);
      if (chain)
             ...
             const char **domain_e_ptr)
{
  int port = 0;
  const char *p;
  const char *colon = (const char *)memchr (domain_b, ':', domain_e - domain_b);
  if (!colon)
    return 0;
...
cookie_jar_save (struct cookie_jar *jar, const char *file)
{
  ...
  for (hash_table_iterate (jar->chains, &iter);
    ...
    {
      const char *domain = (const char *)iter.key;
      struct cookie *cookie = (struct cookie *)iter.value;
...
cookie_jar_delete (struct cookie_jar *jar)
{
  ...
  for (hash_table_iterate (jar->chains, &iter); hash_table_iter_next (&iter); )
    {
      struct cookie *chain = (struct cookie *)iter.value;
      xfree (iter.key);
