...
{
  struct dot_progress *dp = (dot_progress *)xnew0 (struct dot_progress);
...
{
  struct dot_progress *dp = (dot_progress *)progress;
  int dot_bytes = opt.dot_bytes;
...
{
  struct bar_progress *bp = (bar_progress *)xnew0 (struct bar_progress);
  ...
  bp->buffer = (char *)xmalloc (bp->width + 100);
...
{
  struct bar_progress *bp = (bar_progress *)progress;
  ...
  if (received_sigwinch)
    {
      ...
      if (screen_width != old_width)
        {
          bp->width = screen_width - 1;
          bp->buffer = (char *)xrealloc (bp->buffer, bp->width + 100);
...
{
  struct bar_progress::bar_progress_hist *hist = &bp->hist;
