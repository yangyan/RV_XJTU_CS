...
init_interesting (void)
{
  ...
  if (opt.follow_tags)
    {
      ...
      for (followed = opt.follow_tags; *followed; followed++)
        {
          struct known_tag *t = (known_tag *)hash_table_get (interesting_tags, *followed);
          if (!t)
  ...
  for (i = 0; i < countof (additional_attributes); i++)
    hash_table_put (interesting_attributes, additional_attributes[i], (void *)"1");
  for (i = 0; i < countof (tag_url_attributes); i++)
    hash_table_put (interesting_attributes,
                    tag_url_attributes[i].attr_name, (void *)"1");
}
            ...
            struct taginfo *tag, int attrind, struct map_context *ctx)
{
  ...
  newel = (urlpos *)xnew0 (struct urlpos);
  newel->url = url;
...
collect_tags_mapper (struct taginfo *tag, void *arg)
{
  ...
  struct known_tag *t = (known_tag *)hash_table_get (interesting_tags, tag->name);
  assert (t != NULL);
  ...
  while (text < text_end)
    {
      ...
      const char *line_end = (char *)memchr (text, '\n', text_end - text);
      if (!line_end)
        line_end = text_end;
      else
        ++line_end;
      ...
      entry = (urlpos *)xnew0 (struct urlpos);
      entry->url = url;
