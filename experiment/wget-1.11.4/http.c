...
request_new (void)
{
  struct request *req = (struct request *)xnew0 (struct request);
  req->hcapacity = 8;
  req->headers = (request::request_header *)xnew_array (struct request::request_header, req->hcapacity);
  return req;
}
...
static void
release_header (struct request::request_header *hdr)
{
                    ...
                    enum rp release_policy)
{
  ...
  if (req->hcount >= req->hcapacity)
    {
      req->hcapacity <<= 1;
      req->headers = (request::request_header *)xrealloc (req->headers, req->hcapacity * sizeof (*hdr));
    }
...
request_remove_header (struct request *req, char *name)
{
  ...
  for (i = 0; i < req->hcount; i++)
    {
      struct request::request_header *hdr = &req->headers[i];
      if (0 == strcasecmp (name, hdr->name))
        {
          release_header (hdr);
...
resp_new (const char *head)
{
  ...
  struct response *resp = (response *)xnew0 (struct response);
  resp->data = head;
    ...
    if (BOUNDED_EQUAL_NO_CASE (name.b, name.e, "filename") && value.b != NULL)
      {
        ...
        const char *last_slash = (char *)memrchr (value.b, '/', value.e - value.b);
        const char *last_bs = (char *)memrchr (value.b, '\\', value.e - value.b);
        ...
        if (opt.dir_prefix)
          {
            ...
            *filename = (char *)xmalloc (total_length + 1);
  ...
  if (opt.timestamping && !hs->timestamp_checked)
    {
      ...
      char *filename_plus_orig_suffix = (char *)alloca (filename_len + sizeof (".orig"));
      ...
      if (last_period_in_local_filename == NULL
               ...
               || 0 == strcasecmp (last_period_in_local_filename, ".html")))
        {
          ...
          /* Resize the local file, allowing for ".html" preceded by
             optional ".NUMBER".  */
          hs->local_file = (char *)xrealloc (hs->local_file,
